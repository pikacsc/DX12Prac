...
bool InitD3D()
{
	...
	swapChainDesc.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD; // dxgi will discard the buffer (data) afterwe call present
	swapChainDesc.OutputWindow = hwnd; // handle to our window
	swapChainDesc.SampleDesc = sampleDesc; // our multi-sampling description
	swapChainDesc.Windowed = !FullScreen; // set to ture, then if in fullscreen must call SetFullScreenState with true for full screen to get uncapped fps
	...
	// get the size of descriptor in this heap (this is a rtv heap, so only rtv descriptors should be stored in it.
	...
	descriptorTableRanges[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_CBV; // this is range of constant buffer views (descriptors)
	...
	bytecode. We can load the .cso files at runtime to ge the
	...
	*/
	...
	psoDesc.NumRenderTargets = 1; // we are only binding oneone render target
	...
	Vertex vList[] = {
		// first square (closer to camera, blue)
	...
	efficient to copy to default heap where it says on the gpu. In this case, our constant buffer
	...
	*/
	...
	for (int i = 0; i < frameBufferCount; ++i)
	{
		hr = device->CreateCommittedResource(
			...
			D3D12_HEAP_FLAG_NONE, // no flags
			&CD3DX12_RESOURCE_DESC::Buffer(1024 * 64), // size of the resource heap. Must be a multiple of 64KB for single-texutres and constant buffers
	...
	indexBufferView.Format = DXGI_FORMAT_R32_UINT; // 32-bit unsigned integer ( this is what a dword is, double word, a word is 2bytes)
...
void Update()
{
	...
	if (cbColorMultiplierData.colorMultiplier.y >= 1.0f || cbColorMultiplierData.colorMultiplier.y <= 0.0)
	{
		cbColorMultiplierData.colorMultiplier.y = cbColorMultiplierData.colorMultiplier.y >= 1.0f ? 1.0f : 0.0f;
		gIncrement = -gIncrement;
	}
	if (cbColorMultiplierData.colorMultiplier.z >= 1.0f || cbColorMultiplierData.colorMultiplier.z <= 0.0)
	{
...
void UpdatePipeline()
{
	...
	/*
	reset the command list. by resseting the command list we are putting it into 
	a recording state so we can start recording commands into the comman allocator
	the command allocator that we reference here may have multiple command lists
	associated with it, but only one can be recording at any time. Make sure that
	any other command lists associated to this command allocator are in the close state(not recording)
	
	Here you will pass an initial pipeline state object as the second parameter,
	but in this tutorial we are only clearing the rtv, and do not actually need
	anything but an initial defult pipeline, which is what we get by setting
	...
	*/
	...
	warning if present if called on the render target when it's not in the present state
	*/
